...
type getBlogByIdType = (id: string) => Promise<IBlogByIdResDto>;
...
type editBlogByIdType = (_: {
  data: FormData;
  id: string;
}) => Promise<IBlogResDto>;
export const editBlogById: editBlogByIdType = async ({ data, id }) => {
  const response = await axiosInstance.patch(urls.blog.ById(id), data, {
